    ...
    try {
        ...
        for (const item of items) {
            ...
            const itemHTML = `
                ...
                <button class="add-to-cart-btn" data-item-id="${item._id}">Ajouter au panier</button>
            `;
...
document.addEventListener('click', function(event) {
    if (event.target && event.target.classList.contains('add-to-cart-btn')) {
        ...
        const selectedItem = cart.find(item => item._id === itemId); // Utilisez cart au lieu de items
        if (selectedItem) {
